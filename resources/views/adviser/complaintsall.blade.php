...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', function(e) {
    if (e.target.classList.contains('editComplaintBtn')) {
      ...
      if (row) {
        openComplaintModal(`/adviser/complaints/${row.dataset.complaintId}`, {
    ...
    // Edit Appointment Button
      ...
      const row = e.target.closest('tr');
      if (row) {
        openAppointmentModal(`/adviser/complaint-appointments/update/${row.dataset.appId}`, {
    ...
    // Edit Anecdotal Button
      ...
      const row = e.target.closest('tr');
      if (row) {
        openAnecdotalModal(`/adviser/complaint-anecdotals/update/${row.dataset.anecId}`, {
  ...
  async function updateRecordsStatus(selectedItems, status) {
    try {
      const requests = selectedItems.map(item => {
        ...
        switch(item.type) {
          case 'complaint':
            url = `/adviser/complaints/update-status/${item.id}`;
            data = { status: status };
            break;
          case 'appointment':
            url = `/adviser/complaint-appointments/update-status/${item.id}`;
            data = { status: status };
            break;
          case 'anecdotal':
            url = `/adviser/complaint-anecdotals/update-status/${item.id}`;
            data = { status: status };
            break;
        }
  ...
  async function loadArchiveData() {
    try {
      ...
      const complaintResponse = await fetch('/adviser/complaints/archive', {
        headers: { 'X-CSRF-TOKEN': csrfToken }
      });
      archiveData.complaintRecords = await complaintResponse.json();

      // Load appointments archive
      const appointmentResponse = await fetch('/adviser/complaint-appointments/archive', {
        headers: { 'X-CSRF-TOKEN': csrfToken }
      });
      archiveData.complaintAppointments = await appointmentResponse.json();

      // Load anecdotals archive
      const anecdotalResponse = await fetch('/adviser/complaint-anecdotals/archive', {
        headers: { 'X-CSRF-TOKEN': csrfToken }
      });
      ...
      const requests = selectedItems.map(item => {
        ...
        switch(item.type) {
          case 'complaintRecords':
            url = `/adviser/complaints/restore/${item.id}`;
            break;
          case 'complaintAppointments':
            url = `/adviser/complaint-appointments/restore/${item.id}`;
            break;
          case 'complaintAnecdotals':
            url = `/adviser/complaint-anecdotals/restore/${item.id}`;
            break;
        }
  ...
  async function deleteSelectedPermanently(selectedItems) {
    try {
      const requests = selectedItems.map(item => {
        ...
        switch(item.type) {
          case 'complaintRecords':
            url = `/adviser/complaints/delete-permanent/${item.id}`;
            break;
          case 'complaintAppointments':
            url = `/adviser/complaint-appointments/delete-permanent/${item.id}`;
            break;
          case 'complaintAnecdotals':
            url = `/adviser/complaint-anecdotals/delete-permanent/${item.id}`;
            break;
        }
    ...
    showNotification(
      ...
      async function() {
        try {
          ...
          switch(type) {
            case 'complaintRecords':
              url = `/adviser/complaints/restore/${id}`;
              break;
            case 'complaintAppointments':
              url = `/adviser/complaint-appointments/restore/${id}`;
              break;
            case 'complaintAnecdotals':
              url = `/adviser/complaint-anecdotals/restore/${id}`;
              break;
          }
    ...
    showNotification(
      ...
      async function() {
        try {
          ...
          switch(type) {
            case 'complaintRecords':
              url = `/adviser/complaints/delete-permanent/${id}`;
              break;
            case 'complaintAppointments':
              url = `/adviser/complaint-appointments/delete-permanent/${id}`;
              break;
            case 'complaintAnecdotals':
              url = `/adviser/complaint-anecdotals/delete-permanent/${id}`;
              break;
          }

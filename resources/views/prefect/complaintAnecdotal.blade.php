...
<meta name="csrf-token" content="{{ csrf_token() }}">
  ...
  <!-- ✅ Toolbar -->
  <div class="toolbar">
    <h2>Complaint Anecdotal Management</h2>
    <div class="actions">
      <input type="search" placeholder="🔍 Search by student name or ID..." id="searchInput">
      <a href="{{ route('complaint-anecdotal.create') }}" class="btn-primary" id="createAnecBtn">
  ...
  <div class="select-options">
    ...
    <div class="right-controls">
      <!-- Complaint Anecdotals Buttons -->
      <div id="complaintAnecdotalsActions" class="action-buttons">
  ...
  <div class="table-container">
    ...
    <div id="complaintAnecdotalsTable" class="table-wrapper">
      <table>
        <thead>
          <tr>
            <th></th>
        ...
        <tbody>
          @forelse($canecdotals as $anec)
            ...
            <tr
              ...
              data-complainant="{{ $anec->complaint->complainant->student_fname ?? 'N/A' }} {{ $anec->complaint->complainant->student_lname ?? '' }}"
              data-respondent="{{ $anec->complaint->respondent->student_fname ?? 'N/A' }} {{ $anec->complaint->respondent->student_lname ?? '' }}"
              class="clickable-row"
            >
              <td><input type="checkbox" class="rowCheckbox anecdotalCheckbox" value="{{ $anec->comp_anec_id }}"></td>
              <td>{{ $anec->comp_anec_id }}</td>
              <td>
                {{ $anec->complaint->complainant->student_fname ?? 'N/A' }}
                {{ $anec->complaint->complainant->student_lname ?? '' }}
              </td>
              <td>
                {{ $anec->complaint->respondent->student_fname ?? 'N/A' }}
                {{ $anec->complaint->respondent->student_lname ?? '' }}
              </td>
          ...
          <tr class="no-data-row">
            <td colspan="10" style="text-align:center;">No active anecdotal records found</td>
          </tr>
          @endforelse
        </tbody>
      </table>
      ...
      <div class="pagination-wrapper">
        <div class="pagination-summary">
          @if($canecdotals instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php
              $activeCount = $canecdotals->whereIn('status', ['active', 'in_progress'])->count();
            @endphp
  ...
  <div class="modal" id="anecdotalDetailsModal">
    <div class="modal-content">
        ...
        <div class="detail-section">
          ...
          <div class="detail-grid">
            <div class="detail-item">
              ...
              <span id="detail-complainant">-</span>
            </div>
            <div class="detail-item">
              <label>Respondent:</label>
              <span id="detail-respondent">-</span>
            </div>
          </div>
        </div>
      ...
      <div class="modal-actions">
        <button class="btn-primary" id="viewRelatedComplaintBtn">📋 VIEW RELATED COMPLAINT</button>
      </div>
    </div>
  </div>
  ...
  <!-- 🗃️ COMPLAINT ANECDOTALS ARCHIVE MODAL -->
  <div class="modal" id="complaintAnecdotalsArchiveModal">
    <div class="modal-content">
      <div class="modal-header">🗃️ Archived Complaint Anecdotals</div>
      <div class="modal-body">
        <div class="modal-actions">
          <label class="select-all-label">
            <input type="checkbox" id="selectAllComplaintAnecdotalsArchived">
            <span>Select All</span>
          </label>
          <div class="filter-container">
            <select id="complaintAnecdotalsStatusFilter" class="filter-select">
          ...
          <div class="search-container">
            <input type="search" id="complaintAnecdotalsArchiveSearch" placeholder="🔍 Search archived anecdotals..." class="search-input">
          </div>
        </div>

        <div class="archive-table-container">
          <div id="archiveComplaintAnecdotalsTable" class="archive-table-wrapper">
            <table class="archive-table">
              ...
              <tbody id="archiveComplaintAnecdotalsBody">
                <!-- Archived complaint anecdotals will be loaded here via AJAX -->
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div class="modal-footer">
          <button class="btn-secondary" id="restoreComplaintAnecdotalsBtn">🔄 Restore</button>
          <button class="btn-danger" id="deleteComplaintAnecdotalsBtn">🗑️ Delete</button>
          <button class="btn-close" id="closeComplaintAnecdotalsArchive">❌ Close</button>
        </div>
      </div>
    </div>
  </div>
...
let currentTableType = 'complaintAnecdotals';
...
const viewRelatedComplaintBtn = document.getElementById('viewRelatedComplaintBtn');
...
function openAnecdotalDetailsModal(anecdotalData) {
    ...
    document.getElementById('detail-complainant').textContent = anecdotalData.complainant || '-';
    document.getElementById('detail-respondent').textContent = anecdotalData.respondent || '-';
...
document.addEventListener('DOMContentLoaded', function() {
    const anecdotalRows = document.querySelectorAll('#complaintAnecdotalsTable tbody tr.clickable-row');
    ...
    anecdotalRows.forEach(row => {
        row.addEventListener('click', function(e) {
            ...
            const anecdotalData = {
                complainant: this.dataset.complainant,
                respondent: this.dataset.respondent,
...
// View Related Complaint button functionality
viewRelatedComplaintBtn.addEventListener('click', function() {
    const anecdotalId = document.getElementById('detail-anecdotal-id').textContent;
    const complainant = document.getElementById('detail-complainant').textContent;

    alert(`Viewing related complaint for anecdotal ${anecdotalId} - ${complainant}`);
    // Here you would typically navigate to complaints page or load related complaint
    // Example: window.location.href = `/complaints?anecdotal_id=${anecdotalId}`;
});
...
document.addEventListener('click', function(event) {
    const modals = [
        ...
        'editAnecdotalModal',
        'complaintAnecdotalsArchiveModal'
    ];
...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    const tableBody = document.querySelector('#complaintAnecdotalsTable tbody');
...
// ==================== COMPLAINT ANECDOTALS FUNCTIONALITY ====================
...
document.getElementById('markAnecdotalCompletedBtn').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch('/prefect/complaint-anecdotals/archive', {
            method: 'POST',
            headers: {
...
document.getElementById('archiveBtn').addEventListener('click', async function() {
    try {
        console.log('Loading archived complaint anecdotal data');

        // Load archived anecdotals
        const anecdotalResponse = await fetch('/prefect/complaint-anecdotals/archived');
        ...
        populateArchiveTable('archiveComplaintAnecdotalsBody', archivedAnecdotals, 'complaint_anecdotal');
        document.getElementById('complaintAnecdotalsArchiveModal').style.display = 'flex';
...
function populateArchiveTable(tableBodyId, data, type) {
    ...
    data.forEach(item => {
        ...
        if (type === 'complaint_anecdotal') {
            row.setAttribute('data-record-id', item.comp_anec_id);
            row.setAttribute('data-record-type', 'complaint_anecdotal');
            row.innerHTML = `
                <td><input type="checkbox" class="archiveCheckbox" value="${item.comp_anec_id}" data-type="complaint_anecdotal"></td>
                <td>${item.comp_anec_id}</td>
                <td>${item.complainant_fname} ${item.complainant_lname}</td>
                <td>${item.respondent_fname} ${item.respondent_lname}</td>
...
// Archive Search
document.getElementById('complaintAnecdotalsArchiveSearch').addEventListener('input', function() {
    const filter = this.value.toLowerCase();
    const tableBody = document.getElementById('archiveComplaintAnecdotalsBody');
    const rows = tableBody.querySelectorAll('tr');
    rows.forEach(row => {
        const text = row.innerText.toLowerCase();
        row.style.display = text.includes(filter) ? '' : 'none';
...
document.getElementById('complaintAnecdotalsStatusFilter').addEventListener('change', function() {
...
document.getElementById('selectAllComplaintAnecdotalsArchived').addEventListener('change', function() {
    const tableBody = document.getElementById('archiveComplaintAnecdotalsBody');
...
document.getElementById('restoreComplaintAnecdotalsBtn').addEventListener('click', async function() {
    const tableBody = document.getElementById('archiveComplaintAnecdotalsBody');
    ...
    try {
        const response = await fetch('/prefect/complaint-anecdotals/restore-multiple', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
...
document.getElementById('deleteComplaintAnecdotalsBtn').addEventListener('click', async function() {
    const tableBody = document.getElementById('archiveComplaintAnecdotalsBody');
    const selectedCheckboxes = tableBody.querySelectorAll('.archiveCheckbox:checked');
    ...
    try {
        const response = await fetch('/prefect/complaint-anecdotals/destroy-multiple-archived', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
...
// Close Archive Modal
document.getElementById('closeComplaintAnecdotalsArchive').addEventListener('click', function() {
    document.getElementById('complaintAnecdotalsArchiveModal').style.display = 'none';
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.editAnecdotalBtn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      openEditModal(`/prefect/complaint-anecdotals/update/${row.dataset.anecId}`, {
